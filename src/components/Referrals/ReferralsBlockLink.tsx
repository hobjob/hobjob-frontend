...
interface ReferralsBlockLinkProps {
    _id: string;
}

const ReferralsBlockLink: React.FC<ReferralsBlockLinkProps> = ({_id}) => {
    const [stateCopy, setStateCopy] = React.useState(false);

    const inputCopyLinkRef = React.useRef<HTMLInputElement>(null);

    const copyLink = () => {
		if (inputCopyLinkRef.current) inputCopyLinkRef.current.select();
